...
void Manipulation_List (List *L)
{
    ...
    int a;
    if (((INFO(FIRST(*L))) % 2 == 0) && (INFO(FIRST(*L))) > 0)
    {
        InsVFirst(&(*L), INFO(FIRST(*L)));
    }
    else if (((INFO(FIRST(*L))) % 2 != 0) && (INFO(FIRST(*L))) < 0)
    {
        DelVFirst(&(*L), &a);
    }
}
...
{
   // lengkapi fungsinya disini ya , gunakan petunjuk yang ada di main.c :D
   int b,i,jumlah;
